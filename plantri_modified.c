#define VERSION "5.4 - March 1, 2023"
#define SWITCHES "[-uagsETh -Ac#txm#P#bpe#f#qQ -odGVX -v]"
#define TMP

/*

THIS VERSION OF PLANTRI HAS BEEN MODIFIED.
Changes:
- extra arguments added to got_one()
- extra arguments added to scanpoly_c3()
- small amounts of code added to the start of scanpoly_c3() and startpolyscan()
*/
...
static long int res,mod;        /* res/mod from command line (default 0/1) */
static long int splitlevel,
...
static void
got_one(int nbtot, int nbop, int connec, int check_ham, int just_checked)
...
#ifdef FILTER
    if (!FILTER(nbtot,nbop,doflip,check_ham,just_checked)) return;
#endif
 ...
 */
    ...
    got_one(nbtot,nbop,2 + (numdoubles==0),1,0);
   ...
   distinguished by oldnflip==0.  There are no loops yet. */
{
    ...
    if (minconnec == 2) got_one(nbtot,nbop,2+(numdoubles==0),1,0);
...

{
    ...
    if (ne <= edgebound[1]) got_one(nbtot,nbop,3,1,0);
...
scanpoly_c3(int nbtot, int nbop, EDGE *oldfeas[], int noldfeas,
            int oldmaxface, int oldmaxlist0, int oldmaxlist1, int check_ham, int rec_depth)
...

{
	int just_checked = 0;
	if (max_rec_depth != -1) {
		if (check_ham && (rec_depth == 0) && do_truncate) return; 
		if (check_ham && (rec_depth > 0)) {check_ham = ham_snake(); just_checked = 1;} // If predecessor was hamiltonian, recheck
    ...
    if (ne <= edgebound[1]) got_one(nbtot,nbop,3, check_ham, just_checked);
    if (ne == edgebound[0]) return;
    if (ne - 2*noldfeas > edgebound[1]) return;
...
#endif
            {
                if (ngood_or + ngood_inv == 1)
                {
                    ...
                    scanpoly_c3(1,1,newfeas,nnewfeas,
                             newmaxface,newmaxlist0,newmaxlist1,check_ham, rec_depth-1);
                }
                else if (canon_edge_oriented(good_or,ngood_or,ncan_or,
                                             good_inv,ngood_inv,ncan_inv,
                                             degree,numbering,&xnbtot,&xnbop))
   ...
   are given as 3. */
{
    ...
    if (ne <= edgebound[1]) got_one(nbtot,nbop,connec,1,0);
    if (ne == edgebound[0]) return;
    if (ne - 2*noldfeas > edgebound[1]) return;
...

{
	...
	//if (check_ham && (rec_depth == 0)) return; 
   ...
   /* In the -m5 case, the group must be saved and restored. */
    ...
    if (minpolyconnec == 3)
        scanpoly_c3(nbtot,nbop,feasible,nfeas,3,0,0, check_ham, max_rec_depth);
   ...
   removing inequivalent vertices of the required degree. */
{
    ...
    else    /* Case of non-trivial group */
    {
        ...
        for (k = 0; k < ne; ++k)
        {
            ...
            if (connec >= minconnec)
                got_one(newnbtot,newnbop,connec,1,0);
            insert_vertex(v);
            code_edge = NULL;
        }
    }
   ...
   values returned by canon() for that graph. */
{
    ...
    if (nv == maxnv)
    {
        ...
        else if (minconnec == 3)   got_one(nbtot,nbop,3,1,0);
...
*/
{
    ...
    {
        connec = 3 + (numnft==0);
        if (connec >= minconnec) got_one(nbtot,nbop,connec,1,0);
        return;
    }
...
#endif 
        {
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                                                ...
                                                && ngood_mir == ngood_mir_ref)
                {
                    ...
                    if (connec >= minconnec) got_one(1,1,connec,1,0);
                }
                else if (ngood_or+ngood_mir==1)
                {
                    update_nft_P(nft,numnft,extP[i],newnft,&newnumnft);
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                ...
                if (nv == maxnv && !needgroup && ngood_or == ngood_ref
                                              ...
                                              && ((newnbang == 0 && minconnec == 4)
                                                    || minconnec == 5))
                {
                    got_one(1,1,5,1,0);   /* Note: -p implies -G */
                }
                else if (ngood_or+ngood_mir==1)
                {
                    prevA[nprevA] = extAred;
...

{
    ...
    if (nv == maxnv)
    {
        ...
        else         got_one(nbtot,nbop,conn,1,0);
        return;
    }
...

{
    ...
    if (nv == maxnv)
    {
        got_one(nbtot,nbop,4,1,0);   /* Note connectivity is really 3 */
        return;
    }
...

{
    ...
    if (nv == maxnv)
    {
        got_one(nbtot,nbop,1,1,0);  /* Third arg is connectivity but
                                       we don't know it. */
        return;
    }
...

{
    ...
    for (i = 1; !badargs && i < argc; ++i)
    {
        ...
        else
        {
            ...
            else if (sscanf(arg,"%ld/%ld",&ares,&amod) == 2)
            {
                res = ares;
                mod = amod;
            }
